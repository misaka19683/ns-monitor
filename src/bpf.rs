...
pub fn create_ns_filter() -> Vec<libc::sock_filter> {
    vec![
        // Load packet length
        libc::sock_filter { code: 0x80, jt: 0, jf: 0, k: 0 },       // BPF_LD + BPF_W + BPF_LEN
        // Check if packet is large enough (at least 54 bytes for Eth+IPv6+ICMPv6)
        libc::sock_filter { code: 0x35, jt: 0, jf: 4, k: 54 },      // BPF_JMP + BPF_JGE + BPF_K
        // Load Ethernet Type field (2 bytes at offset 12)
        libc::sock_filter { code: 0x28, jt: 0, jf: 0, k: 12 },      // BPF_LD + BPF_H + BPF_ABS
        // Jump if not IPv6 (0x86dd)
        libc::sock_filter { code: 0x15, jt: 0, jf: 2, k: 0x86dd },  // BPF_JMP + BPF_JEQ + BPF_K
        // Load IPv6 Next Header field (1 byte at offset 20)
        libc::sock_filter { code: 0x30, jt: 0, jf: 0, k: 20 },      // BPF_LD + BPF_B + BPF_ABS
        // Jump if ICMPv6 (58)
        libc::sock_filter { code: 0x15, jt: 1, jf: 0, k: 58 },      // BPF_JMP + BPF_JEQ + BPF_K
        // Reject packet
        libc::sock_filter { code: 0x06, jt: 0, jf: 0, k: 0 },       // BPF_RET + BPF_K
        // Accept packet
        libc::sock_filter { code: 0x06, jt: 0, jf: 0, k: 0xffff },  // BPF_RET + BPF_K
    ]
}
            ...
            while pc < program.len() && iterations < max_iterations {
                ...
                match class {
                    0x00 => { // BPF_LD
                        ...
                        match (size, mode) {
                            (0x00, 0x20) => { // BPF_W + BPF_ABS - load word absolute
                            ...
                            (0x08, 0x20) => { // BPF_H + BPF_ABS - load half-word absolute
                                let offset = k as usize;
                            ...
                            (0x10, 0x20) => { // BPF_B + BPF_ABS - load byte absolute
                                let offset = k as usize;
                            ...
                            (0x00, 0x80) => { // BPF_W + BPF_LEN - load packet length
...
mod bpf_tests {
    ...
    fn test_bpf_filter_structure() {
        ...
        assert_eq!(filter[0].code, 0x80, "First instruction should be BPF_LD + BPF_W + BPF_LEN");
        assert_eq!(filter[0].k, 0, "LEN instruction should have k=0");
        
        // Test second instruction: check minimum length
        assert_eq!(filter[1].code, 0x35, "Second instruction should be BPF_JMP + BPF_JGE + BPF_K");
        assert_eq!(filter[1].k, 54, "Should check for minimum 54 bytes");
        assert_eq!(filter[1].jf, 4, "Should jump forward 4 instructions on failure");
        
        // Test EtherType check
        assert_eq!(filter[2].code, 0x28, "Third instruction should load half-word");
    ...
    fn test_bpf_vm_basic_operations() {
        ...
        let simple_program = vec![
            libc::sock_filter { code: 0x80, jt: 0, jf: 0, k: 0 },     // BPF_LD + BPF_LEN
            libc::sock_filter { code: 0x06, jt: 0, jf: 0, k: 42 },    // BPF_RET + BPF_K (return 42)
        ];
    ...
    fn test_bpf_vm_infinite_loop_protection() {
        ...
        let infinite_loop_program = vec![
            libc::sock_filter { code: 0x80, jt: 0, jf: 0, k: 0 },     // BPF_LD + BPF_LEN
            libc::sock_filter { code: 0x15, jt: 0, jf: 0, k: 100 },   // BPF_JEQ - always jump to self
        ];
