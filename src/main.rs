...
async fn monitor_ns_packets(socket: Socket, state: Arc<Mutex<AppState>>) -> Result<()> {
    ...
    loop {
        ...
        let buf_slice = unsafe { MaybeUninit::slice_assume_init_ref(&buf[..len]) };
...
//                     if let Some((source_mac, target)) =
//                         ndp::validate_outgoing_ns_packet(&buf[..len])
//                     {
...
//                                 info!(
//                                     "📡 Detected outgoing NS packet from {} for target {} - forwarding to slave interfaces",
//                                     source_mac, target
//                                 );
//
...
//                         } else {
//                             debug!(
//                                 "Ignoring outgoing NS packet from different MAC {} (expected {})",
